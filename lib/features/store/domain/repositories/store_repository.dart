...
class StoreRepository implements StoreRepositoryInterface {
  ...
  Future<StoreModel?> _getStoreList(
    ...
    String cacheId =
        '${AppConstants.storeUri}/$filterBy?store_type=$storeType&offset=$offset&limit=10-${Get.find<SplashController>().module!.id!}';

    switch (source) {
      case DataSourceEnum.client:
        Response response = await apiClient.getData(
            '${AppConstants.storeUri}/$filterBy?store_type=$storeType&offset=$offset&limit=10');
    ...
    switch (source) {
      case DataSourceEnum.client:
        ...
        if (response.statusCode == 200) {
          ...
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body['stores']), apiClient.getHeader());
        }
  ...
  Future<List<Store>?> _getFeaturedStoreList(
    ...
    String cacheId =
        '${AppConstants.storeUri}/all?featured=1&offset=1&limit=10-${Get.find<SplashController>().module?.id ?? ''}';
    Map<String, String> header = (Get.find<SplashController>().module == null &&
    ...
    switch (source) {
      case DataSourceEnum.client:
        ...
        if (response.statusCode == 200) {
          ...
          LocalClient.organize(DataSourceEnum.client, cacheId,
              jsonEncode(response.body['stores']), header);
        }
  ...
  Future<RecommendedItemModel?> _getStoreRecommendedItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.storeRecommendedItemUri}?store_id=$storeId&offset=1&limit=10');
    if (response.statusCode == 200) {
  ...
  Future<CartSuggestItemModel?> getCartStoreSuggestedItemList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.cartStoreSuggestedItemsUri}?recommended=1&store_id=$storeId&offset=1&limit=10',
        headers: header);
    if (response.statusCode == 200) {
