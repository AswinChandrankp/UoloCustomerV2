...
class ItemsView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bool isEmpty = isStore
        ? (stores == null || stores!.isEmpty)
        : (items == null || items!.isEmpty);

    if (isHome) {
      return _HomeStoreList(stores: stores);
    }

    if (isEmpty) {
      return _ShimmerOrNoData(
        isStore: isStore,
    ...
    if (isStore) {
      return _StoreGrid(
        ...
        fromGroceryCategory: fromGroceryCategory,
        isFoodOrGrocery: isFoodOrGrocery ?? true,
      );
    }

    if (inStorePage) {
      return _InStoreItemsSection(
        items: items!,
        isScrollable: isScrollable,
...
class _HomeStoreList extends StatelessWidget {
  final List<Store?>? stores;

  const _HomeStoreList({required this.stores});

  @override
  Widget build(BuildContext context) {
    return ListView.builder(
      itemCount: stores?.length ?? 10,
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemBuilder: (context, index) {
        if (stores == null) return const StoreCardShimmer2();

        if (index % 7 == 3) {
          return const VisitAgainView(fromFood: true, itemsview: true);
        } else if (index % 7 == 6) {
...
class _StoreGrid extends StatelessWidget {
  ...
  const _StoreGrid({
    ...
    required this.fromGroceryCategory,
    required this.isFoodOrGrocery,
  });
    ...
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: ResponsiveHelper.isMobile(context)
            ? (fromGroceryCategory ? 2 : 1)
            : 3,
        mainAxisExtent: fromGroceryCategory ? 270 : 200,
        mainAxisSpacing: 10,
        crossAxisSpacing: 10,
      ),
      itemCount: stores.length,
      itemBuilder: (context, i) {
        return isFoodOrGrocery
            ? StoreCardWidget2(store: stores[i])
            : StoreCardWithDistance(store: stores[i]!, fromAllStore: true);
      },
    );
  }
}

class _InStoreItemsSection extends StatelessWidget {
  final List<Item?> items;
  final bool isScrollable;
  final bool isFeatured;
  final bool isCampaign;

  const _InStoreItemsSection({
    required this.items,
    required this.isScrollable,
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        GetBuilder<StoreController>(builder: (ctrl) {
          if (ctrl.recommendedItemModel?.items?.isEmpty ?? true) {
          ...
          return Container(
            ...
            child: Column(
              children: [
                InkWell(
                  onTap: ctrl.togglerecommendedContainer,
                  child: Container(
                    height: 45,
                ...
                if (!ctrl.isopen)
                  ListView.builder(
                    ...
                    shrinkWrap: true,
                    physics: const NeverScrollableScrollPhysics(),
                    itemCount: ctrl.recommendedItemModel!.items!.length,
                    itemBuilder: (context, i) => Padding(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 10, vertical: 5),
                      child: ItemWidget(
                        item: ctrl.recommendedItemModel!.items![i],
                        isStore: false,
                        store: null,
                        index: i,
                        length: ctrl.recommendedItemModel!.items!.length,
                        // Recomended: true,
                      ),
                    ),
                  ),
...
class _ItemGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // final sorted = [...available, ...notAvailable];
    ...
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: ResponsiveHelper.isMobile(context)
            ? (fromGroceryCategory ? 2 : 1)
            : 3,
        mainAxisExtent: fromGroceryCategory ? 270 : 210,
        mainAxisSpacing: 10,
        crossAxisSpacing: 20,
      ),
      itemCount: items.length,
      itemBuilder: (context, i) {
        return fromGroceryCategory
            ? ItemCard(
                item: items[i]!, isFood: false, isShop: false, width: 200)
            : Column(
                children: [
                  ItemWidget(
                      item: items[i],
...
class _ShimmerOrNoData extends StatelessWidget {
  final bool isStore;
  final int shimmerLength;
  final EdgeInsetsGeometry padding;
  final String? noDataText;

  const _ShimmerOrNoData({
    required this.isStore,
  ...
  Widget build(BuildContext context) {
    ...
    final String text = noDataText ??
        (isStore
            ? (config.moduleConfig!.module!.showRestaurantText!
