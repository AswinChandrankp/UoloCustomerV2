...
class OrderDetailsScreenState extends State<OrderDetailsScreen> {
          ...
          return orderController.orderDetails != null &&
              ...
              Column(
                  children: [
                          ...
                          SliverToBoxAdapter(
                            child: ResponsiveHelper.isDesktop(context)
                                ...
                                : OrderCalculationWidget(
                                    ...
                                    itemsPrice: _calculateItemsPrice(
                                    ...
                                    subTotal: _calculateSubTotal(
                                        orderController, order),
                                    total:
                                        _calculateTotal(orderController, order),
                    ...
                    ResponsiveHelper.isDesktop(context)
                        ? const SizedBox()
                        : _bottomView(orderController, order, parcel,
                            _calculateTotal(orderController, order)),
                  ],
                )
            ...
            child: FooterView(
              child: SizedBox(
                  ...
                  child: Stack(children: [
                          ...
                          onMapCreated: (GoogleMapController controller) {
                            ...
                            setMarker(
                              order!.orderType == 'parcel'
                                  ? Store(
                                      latitude: order.receiverDetails!.latitude,
                                      longitude:
                                          order.receiverDetails!.longitude,
                                      address: order.receiverDetails!.address,
                                      name: order
                    ...
                    Positioned(
                      right: 15,
                      bottom: order.orderType != 'take_away' &&
                        ...
                        onTap: () => _checkPermission(() async {
                          ...
                          setMarker(
                            order.orderType == 'parcel'
                                ? Store(
                                    latitude: order.receiverDetails!.latitude,
                                    longitude: order.receiverDetails!.longitude,
                                    address: order.receiverDetails!.address,
                                    name: order
                                        .receiverDetails!.contactPersonName)
                                : order.store,
                ...
                child: Row(children: [
                  ((order.orderStatus == 'pending' &&
                      ...
                      ? Expanded(
                          child: CustomButton(
                            buttonText:
                                parcel ? 'track_delivery'.tr : 'Message'.tr,
  ...
  double _calculateItemsPrice(
      OrderController orderController, OrderModel order) {
    double itemsPrice = 0;
    if (order.prescriptionOrder!) {
      double orderAmount = order.orderAmount ?? 0;
      itemsPrice = (orderAmount + _calculateDiscount(order)) -
          ((order.taxStatus! ? 0 : order.totalTaxAmount!) +
  ...
  double _calculateDiscount(OrderModel order) {
    return order.storeDiscountAmount! +
  ...
  double _calculateSubTotal(OrderController orderController, OrderModel order) {
    return _calculateItemsPrice(orderController, order) +
        _calculateAddOns(orderController);
  }

  double _calculateTotal(OrderController orderController, OrderModel order) {
    return _calculateSubTotal(orderController, order) -
  ...
  void setMarker(Store? store, DeliveryMan? deliveryMan,
      AddressModel? addressModel, bool takeAway, bool parcel, bool isRestaurant,
