...
//              storeModel!.stores!.removeWhere((element) =>  element.noservicerestriction == 0 &&  element.distancelimit == 0  && store!.zoneId ==   Get.find<LocationController>().zoneID );
//                  update();
...
//        print("==============================================================  zone id ${Get.find<LocationController>().zoneID}");
//       update();
//     }
//   }
...
//           _popularStoreList = [];
//           popularStoreList.removeWhere((element) =>  element.noservicerestriction == 0 &&  element.distancelimit == 0  && store!.zoneId ==   Get.find<LocationController>().zoneID  );
...
//       latestStoreList.removeWhere((element) => element.noservicerestriction == 0 && element.distancelimit == 0 && store!.zoneId ==   Get.find<LocationController>().zoneID);
//           update();
...
//     if (storeItemModel != null) {
//       // Sort items by availability before adding to the model
//       List<Item>? sortedItems = _sortItemsByAvailability(storeItemModel.items);
...
//         // For pagination, we need to merge and re-sort all items
//         List<Item>? allItems = [...(_storeItemModel!.items ?? []), ...sortedItems!];
//         List<Item>? finalSortedItems = _sortItemsByAvailability(allItems);
...
// // Helper method to sort items by availability
// List<Item>? _sortItemsByAvailability(List<Item?>? items) {
...
//   // Return available items first, then unavailable items
...
//   print('zoneid in store controller $Zoneid');
//   update();
// }
